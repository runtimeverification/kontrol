...
def exec_load_state(options: LoadStateOptions) -> None:
    foundry_state_load(
        options=options,
        foundry=_load_foundry(options.foundry_root, add_enum_constraints=not options.omit_enum_constraints),
    )
    ...
    try:
        ...
        results = foundry_prove(
            foundry=_load_foundry(
                options.foundry_root, options.bug_report, add_enum_constraints=not options.omit_enum_constraints
            ),
            options=options,
...
def exec_show(options: ShowOptions) -> None:
    output = foundry_show(
        foundry=_load_foundry(
            ...
            add_enum_constraints=not options.omit_enum_constraints,
        ),
        options=options,
    )
    print(output)
...
def exec_view_kcfg(options: ViewKcfgOptions) -> None:
    foundry = _load_foundry(
        options.foundry_root, use_hex_encoding=True, add_enum_constraints=not options.omit_enum_constraints
    )
...
def exec_simplify_node(options: SimplifyNodeOptions) -> None:
    ...
    pretty_term = foundry_simplify_node(
        foundry=_load_foundry(
            options.foundry_root, options.bug_report, add_enum_constraints=options.omit_enum_constraints
        ),
        options=options,
    )
    print(f'Simplified:\n{pretty_term}')
