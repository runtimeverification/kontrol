...
contract ArithmeticTest is Test {
    ...
    function test_wmul_increasing(uint a, uint b) public pure {
        if (b <= MAX_INT / a) {
            uint c = wmul2(a, b);
            assertTrue(a < c && b < c);
        }
    ...
    function test_wmul_increasing_positive(uint a, uint b) public pure {
        if (0 < a && 0 < b) {
            if (b <= MAX_INT / a) {
                uint c = wmul2(a, b);
                assertTrue(a < c && b < c);
    ...
    function test_wmul_increasing_gt_one(uint a, uint b) public pure {
        if (WAD < a && WAD < b) {
            if (b <= MAX_INT / a) {
                uint c = wmul2(a, b);
                assertTrue(a < c && b < c);
    ...
    function test_wmul_weakly_increasing_positive(uint a, uint b) public pure {
        if (0 < a && 0 < b) {
            if (b <= MAX_INT / a) {
                uint c = wmul2(a, b);
                assertTrue(a <= c && b <= c);
    ...
    // not passing
    function test_wmul_wdiv_inverse(uint a, uint b) public pure {
        if (WAD < a && WAD < b) {
            if (b <= MAX_INT / a) {
                uint c = wdiv2(wmul2(a, b), b);
                assertEq(a, c);
    ...
    function test_wdiv_rounding(uint a, uint b) external pure {
        uint c1 = wdiv1(a, b);
        uint c2 = wdiv2(a ,b);
        assertTrue(c2 <= c1);
    }

    function test_decreasing_div(uint a, uint b) external pure {
